...
export default function LiveBroadcast() {
  ...
  // Forcer une connexion fraîche avec le token de l'hôte
  ...
  // Enregistrer dans la room socket dès l'ouverture
  useEffect(() => {
    if (!id) return

    const registerHost = () => {
      console.log('[LiveBroadcast] emit live:start pour liveId:', id)
      emit('live:start', { liveId: id })
    }

    const t = setTimeout(registerHost, 600)
    const offConnect  = on('connect',      registerHost)
    const offStarted  = on('live:started', ({ liveId }) => console.log('[LiveBroadcast] ✅ live:started reçu', liveId))
    const offError    = on('error',        ({ message }) => console.error('[LiveBroadcast] ❌ socket error:', message))
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            {live && (
              <p className="text-xs text-slate-400 mt-0.5 truncate max-w-sm">{live.title}</p>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-5">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* Player */}
            <div className="relative rounded-2xl overflow-hidden bg-slate-900 aspect-video shadow-xl shadow-slate-200">
              ...
              {streaming && (
                <div className="absolute bottom-0 left-0 right-0 px-4 py-3 bg-gradient-to-t from-black/70 to-transparent flex items-center gap-2">
                  {/* Micro */}
                  <button onClick={toggleMic} title={micOn ? 'Couper le micro' : 'Activer le micro'}
                  ...
                  {/* Caméra */}
                  <button onClick={toggleCam} title={camOn ? 'Couper la caméra' : 'Activer la caméra'}
                    className={`w-9 h-9 rounded-xl flex items-center justify-center transition ${
                  ...
                  {live.properties.map(lp => (
                    <button key={lp.id}
                      ...
                      className={`flex-shrink-0 w-44 rounded-xl overflow-hidden border text-left transition-all ${
                        lp.isActive
                          ? 'border-blue-400 ring-1 ring-blue-200 shadow-md shadow-blue-100'
