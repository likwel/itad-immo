...
const LIVE_STREAMS = [
  { id:1, viewers:247, duration:'32 min', title:'Visite exclusive — Villa moderne Analamahitsy', host:{ name:'Martin Rakoto', agency:'Prestige Immo', initials:'MR', color:'#2563eb', verified:true }, tags:['Villa','Vente'], properties:3 },
  { id:2, viewers:89,  duration:'14 min', title:'Appartements neufs — Résidence Ivandry Park',  host:{ name:'Sofia Andriamasy', agency:'Immo Concept', initials:'SA', color:'#7c3aed', verified:true }, tags:['Appartement','Vente'], properties:4 },
  { id:3, viewers:51,  duration:'8 min',  title:'Location meublée — Ambohijatovo centre',       host:{ name:'Ony Immo', agency:'Ony Immobilier', initials:'OI', color:'#0891b2', verified:false }, tags:['Studio','Location'], properties:2 },
  { id:4, viewers:51,  duration:'8 min',  title:'Location meublée — Ambohijatovo centre',       host:{ name:'Ony Immo', agency:'Ony Immobilier', initials:'OI', color:'#0891b2', verified:true }, tags:['Studio','Location'], properties:2 },
  { id:5, viewers:51,  duration:'8 min',  title:'Location meublée — Ambohijatovo centre',       host:{ name:'Ony Immo', agency:'Ony Immobilier', initials:'OI', color:'#0891b2', verified:false }, tags:['Studio','Location'], properties:2 },
]

const UPCOMING_STREAMS = [
  { id:10, scheduledAt:"Aujourd'hui · 18h00", title:'Terrain constructible 2000m² — Ambohidratrimo', host:{ name:'Fara Rakoto', agency:'Terra Immo', initials:'FR', color:'#059669', verified:false }, tags:['Terrain','Vente'] },
  { id:11, scheduledAt:'Demain · 10h00', title:'Villas de vacances — Nosy Be, saison 2025', host:{ name:'Paradise Estates', agency:'Paradise Estates', initials:'PE', color:'#d97706', verified:true }, tags:['Villa','Vacances'] },
  { id:12, scheduledAt:'Samedi · 14h00', title:'Bureaux premium — Zone Galaxy, Andraharo', host:{ name:'City Office', agency:'City Office Immo', initials:'CO', color:'#6366f1', verified:true }, tags:['Bureau','Location'] },
]

const PAST_STREAMS = [
  { id:20, views:1240, duration:'1h 12min', daysAgo:'Hier',            title:'Résidence haut standing — Ambatobe, 5 villas', host:{ name:'Prestige Immo',    initials:'PI', color:'#2563eb', verified:true  }, tags:['Villa','Vente'],       properties:5 },
  { id:21, views:876,  duration:'48 min',   daysAgo:'Il y a 2 jours',  title:'Open house — Appartements Talatamaty',         host:{ name:'Sofia Andriamasy', initials:'SA', color:'#7c3aed', verified:true  }, tags:['Appartement','Vente'], properties:6 },
  { id:22, views:534,  duration:'35 min',   daysAgo:'Il y a 3 jours',  title:'Maisons à louer — Ankaditoho',                 host:{ name:'Ony Immo',         initials:'OI', color:'#0891b2', verified:false }, tags:['Maison','Location'],   properties:3 },
  { id:23, views:412,  duration:'28 min',   daysAgo:'Il y a 5 jours',  title:'Terrains viabilisés — Itaosy, lots disponibles',host:{ name:'Terra Immo',       initials:'TI', color:'#059669', verified:false }, tags:['Terrain','Vente'],     properties:8 },
  { id:24, views:298,  duration:'22 min',   daysAgo:'Il y a 1 semaine',title:'Studio meublé — Proche université Ankatso',    host:{ name:'Fara Rakoto',      initials:'FR', color:'#d97706', verified:false }, tags:['Studio','Location'],   properties:2 },
  { id:25, views:187,  duration:'19 min',   daysAgo:'Il y a 2 sem.',   title:'Villas bord de mer — Mahajanga, 3 biens',      host:{ name:'Paradise Estates', initials:'PE', color:'#d97706', verified:true  }, tags:['Villa','Vente'],       properties:3 },
]
...
function LiveCard({ stream, featured, onClick }) {
  ...
  return (
    <div onClick={onClick}
      ...
      {/* Thumbnail */}
      <div style={{
        height: featured ? 210 : 195, position:'relative',
        background:`linear-gradient(135deg, ${stream.host.color}10, ${stream.host.color}04)`,
        display:'flex', alignItems:'center', justifyContent:'center',
        ...
        <div style={{
          ...
          background: stream.host.color, border:'3px solid #fff',
          display:'flex', alignItems:'center', justifyContent:'center',
          boxShadow:`0 8px 24px ${stream.host.color}45`, position:'relative', zIndex:1,
          transform: hovered ? 'scale(1.08)' : 'scale(1)', transition:'transform .2s',
        }}>
      ...
      <div style={{ padding:'14px 16px 16px' }}>
        <div style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <p style={{ fontSize:12, color:'#94a3b8', margin:0 }}>{stream.host.agency}</p>
          </div>
        </div>
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{
        ...
        borderRadius:14, border:`1px solid ${hovered ? '#cbd5e1' : '#e2e8f0'}`,
      ...
      <button onClick={e => { e.stopPropagation(); onToggle() }} style={{
        ...
        border: notified ? '1.5px solid #bfdbfe' : '1.5px solid #e2e8f0',
        background: notified ? '#eff6ff' : '#f8fafc',
        color: notified ? '#2563eb' : '#64748b',
  ...
  return (
    <div onClick={onClick}
      ...
      style={{
        ...
        border:`1px solid ${hovered ? '#cbd5e1' : '#e2e8f0'}`, background:'#fff',
        cursor:'pointer', transition:'all .18s',
        transform: hovered ? 'translateY(-2px)' : 'translateY(0)',
        boxShadow: hovered ? '0 6px 20px rgba(0,0,0,0.07)' : 'none',
      }}>
      <div style={{
        width:110, height:72, borderRadius:10, flexShrink:0,
        background:`linear-gradient(135deg, ${stream.host.color}12, ${stream.host.color}05)`,
        display:'flex', alignItems:'center', justifyContent:'center',
        position:'relative', border:'1px solid #f1f5f9', overflow:'hidden',
      }}>
      ...
      <div style={{ flex:1, minWidth:0, display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:8 }}>
          <span style={{ fontSize:11, color:'#94a3b8', display:'flex', alignItems:'center', gap:3 }}>
            <Icon d={Icons.eye} size={11}/>{stream.views.toLocaleString('fr')} vues
          </span>
...
export default function LiveList() {
  ...
  const match = list => list.filter(s => {
    ...
    const ok1 = !q || s.title.toLowerCase().includes(q) || (s.host.agency || s.host.name || '').toLowerCase().includes(q)
    const ok2 = filter === 'Tous' || s.tags.includes(filter)
    return ok1 && ok2
  })

  const lives    = match(LIVE_STREAMS)
  const upcoming = match(UPCOMING_STREAMS)
  const past     = match(PAST_STREAMS)
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f8fafc', fontFamily:"'DM Sans', system-ui, sans-serif" }}>
      ...
      {/* ── Hero ── */}
      <div style={{ background:'#fff', borderBottom:'1px solid #e2e8f0', padding:'40px 24px 28px' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16 }}>
            <div style={{ display:'flex', alignItems:'center', gap:6, background:'#fef2f2', border:'1px solid #fecaca', padding:'4px 12px', borderRadius:20 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color:'#dc2626', letterSpacing:'0.06em' }}>
                {LIVE_STREAMS.length} LIVE EN COURS
              </span>
            </div>
          </div>
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'36px 24px 60px' }}>
        ...
        {lives.length > 0 && (
          <section style={{ marginBottom:48 }}>
            <SectionTitle accent="#ef4444" label="En direct maintenant" count={lives.length} countColor="#ef4444" countBg="#fef2f2" countBorder="#fecaca"/>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:16 }}>
              {lives.map((stream, i) => (
                <div key={stream.id} className="lu-item" style={{ animationDelay:`${i*0.06}s`, gridColumn: i === 0 && lives.length > 1 ? 'span 2' : 'span 1' }}>
                  <LiveCard stream={stream} featured={i === 0 && lives.length > 1} onClick={() => navigate(`/live/${stream.id}`)}/>
                </div>
              ))}
            </div>
          </section>
        ...
        {upcoming.length > 0 && (
          <section style={{ marginBottom:48 }}>
            <SectionTitle accent="#2563eb" label="Prochains lives" count={`${upcoming.length} programmés`} countColor="#2563eb" countBg="#eff6ff" countBorder="#bfdbfe"/>
        ...
        {past.length > 0 && (
          <section>
            <SectionTitle accent="#94a3b8" label="Replays disponibles" count={`${past.length} vidéos`}/>
        ...
        {/* Empty */}
        {!lives.length && !upcoming.length && !past.length && (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'80px 0', gap:12 }}>
            <div style={{ width:56, height:56, borderRadius:16, background:'#f1f5f9', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <Icon d={Icons.video} size={24} className="" style={{ color:'#94a3b8' }}/>
            </div>
